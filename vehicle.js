...
class Vehicle {
  ...
  // Créer une copie du vehicule courant 
  copy() {
    let newVehicle = new Vehicle(this.pos.x, this.pos.y);
    newVehicle.vel = this.vel.copy();
    newVehicle.acc = this.acc.copy();
    newVehicle.maxSpeed = this.maxSpeed;
    newVehicle.maxForce = this.maxForce;
    newVehicle.r_pourDessin = this.r_pourDessin;
    newVehicle.color = this.color;
    return newVehicle;
  }
  ...
  applyBehaviors(target, obstacles, vehicules , d = 0) {
    ...
    let seekForce = this.arrive(target, d);    
    let avoidForce = this.avoid(obstacles);
    let separateForce = this.separate(vehicules);

    seekForce.mult(0.2);
    avoidForce.mult(3);
    separateForce.mult(2); // 0.3
  ...
  arrive(target , d) {
    ...
    return this.seek(target, d , true);
  }

  seek(target, d = 0, arrival = false) {
...
class Target extends Vehicle {
  constructor(x, y) {
    ...
    {
      push();
      translate(this.pos.x, this.pos.y); // Positionner à l'emplacement de l'objet
    
      // // Dessiner le cercle
      // if (this.showCircle) { // Condition optionnelle pour afficher le cercle
      //   stroke(255); // Couleur du contour
      //   strokeWeight(2);
      //   fill("#F063A4"); // Couleur de remplissage
      //   circle(0, 0, this.r * 2);
      // }
    
      // Dessiner l'image
      if (this.image) { // Vérifie si une image est associée à l'objet
        let angle = this.vel.heading() + PI / 2; // Calcul de l'orientation
        rotate(angle); // Orienter l'image selon la direction de la vitesse
        imageMode(CENTER);
        image(this.image, 0, 0, this.r * 2, this.r * 2); // Dessiner l'image
      }
